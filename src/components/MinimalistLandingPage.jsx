...
import { MicrophoneIcon, DocumentTextIcon, ArrowPathIcon, CheckCircleIcon, UserIcon, StarIcon, HeartIcon
...
return (
  ...
  <div className="min-h-screen bg-gradient-to-br from-background via-muted/30 to-primary/5 text-foreground font-sans relative overflow-hidden">
    {/* Modern mesh gradient background */}
    <div className="absolute inset-0 mesh-gradient pointer-events-none" />
    <div className="absolute inset-0 grid-pattern opacity-[0.03] pointer-events-none" />
    {/* Modern Navigation Header with Glassmorphism */}
    <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-border animate-slide-up">
    ...
    <div className="flex items-center space-x-4">
            ...
            <a href="/dashboard" className="px-4 py-2 text-sm font-medium text-foreground/70 hover:text-foreground transition-smooth hover-scale" >
 Dashboard 
            </a>
    <button onClick={handleDemoClick} className="btn-medical btn-medical-primary text-sm" >
    {t('try_demo')}
    </button>
    </div>
    </div>
    ...
    {/* Demo Mode Banner */} {isDemoMode && <DemoModeBanner />} {/* Main Content */}
    ...
    <div className="card-gradient glass rounded-3xl p-12 backdrop-blur-xl border border-white/20 shadow-2xl">
    ...
    <div className="mb-8">
    <span itemScope itemType="image/png" itemProp="image">
    <a itemProp="url" href="/banner1.png"/>
    </span>
                  <Image src="/banner1.png" alt="SYMFARMIA Medical Platform" width={1200} height={600} priority placeholder="blur" blurDataURL="data:image/png;
                base64,iVBORw0KGgoAAAANSUhEUgAABAAAAAQACAIAAADwf7zUAAEw2mNhQlgAATDaanVtYgAAAB5qdW1kYzJwYQARABCAAACqADibcQNjMnBhAAAANxNqdW1iAAAA" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
    </div>
    {/* CTA Buttons */}
    <div className="mb-12">
    {!isDemoMode && !isSubmitted ? ( <div className="space-y-6">
    <form onSubmit={handleSubmit} className="max-w-md mx-auto">
    <div className="flex flex-col sm:flex-row gap-4">
    
                      <input type="email" value={email} onChange={(e) =>
                       setEmail(e.target.value)} placeholder="tu-email@ejemplo.com" required className="input-modern flex-1 text-base shadow-sm" />
    
                        <button type="submit" disabled={isSubmitting} className="btn-medical btn-medical-primary disabled:opacity-50 disabled:cursor-not-allowed" >
    ...
    <div className="flex justify-center">
                      ...
                      <button onClick={handleDemoClick} className="btn-medical btn-medical-secondary glass border-2 border-primary/30 text-primary hover:border-primary/50" >
    {t('demo_interactive')}
    </button>
    </div>
    </div> ) : isDemoMode ? ( <div className="text-center py-8 animate-fade-in">
    ...
    <div className="card-float glass p-8 text-center animate-slide-up" style={{ animationDelay: '200ms' }}>
 ...
 3 
    ...
    {!isSubmitted && !isDemoMode && ( <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
    
                      <button onClick={() =>
                       document.querySelector('input[type="email"]').scrollIntoView()} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-8 py-3 rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105" >
    {t('final_cta_signup')}
    </button>
    
                      <button onClick={handleDemoClick} className="bg-white/80 hover:bg-white border-2 border-blue-200 hover:border-blue-300 text-blue-600 font-semibold px-8 py-3 rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105" >
    {t('final_cta_demo')}
    </button>
    </div> )}
    </div>
