...
import { 
  X, User, Mail, Phone, Calendar, FileText, UserCheck, CheckCircle, AlertCircle
} from 'lucide-react';
...
const handleSubmit = (e) => {
    ...
    const submitData = { ...formData, dateOfBirth: new Date(formData.dateOfBirth) };
    onSave(submitData);
  };
...
if (!isOpen) return null;
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 opacity-100 @starting-style:opacity-0 transition-opacity duration-300 ease-out">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden scale-100 translate-y-0 @starting-style:scale-95 @starting-style:translate-y-4 transition-all duration-300 ease-out border border-gray-200/50 relative">
        {/* Gradient overlay */}
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200/50 relative z-10">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-gradient-to-br from-medical-primary/20 to-medical-accent/20 rounded-xl flex items-center justify-center shadow-inner">
          ...
          <button
            onClick={handleClose}
            className="group p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-all duration-300"
            disabled={isLoading}
          >
