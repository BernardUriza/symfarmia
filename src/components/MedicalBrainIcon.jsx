...
const MedicalBrainIcon = ({ className = '' }) => {
  const { toggleAssistant, assistantOpen, clinicalAlerts, activePatient } =
    usePatientContext();

  const hasHighPriorityAlerts = clinicalAlerts.some(
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.95 }}
      className={`relative p-2 rounded-lg transition-all duration-200 ${assistantOpen ? 'bg-blue-100 text-blue-600 shadow-md' : 'bg-white text-gray-600 hover:bg-gray-50 hover:text-blue-600 '} ${className}`}
