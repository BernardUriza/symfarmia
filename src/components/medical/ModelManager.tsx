  ...
  return (
    <div className="p-4 border border-gray-200 rounded-lg mb-6 bg-white">
      ...
      <form onSubmit={(e) => { e.preventDefault(); saveItem(); }} className="space-y-6 mb-8">
        ...
        <button
          type="submit"
          className="min-h-[44px] bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg transition-colors"
        >
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {items.map(item => (
              <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                {fieldDefs.map(fd => (
                  <td key={fd.name} className="px-6 py-4 whitespace-nowrap text-gray-900">
                    {String(item[fd.name] ?? '')}
                  </td>
                ))}
...
// Add default export to fix the Netlify build error
