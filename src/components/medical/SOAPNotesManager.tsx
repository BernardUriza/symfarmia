...
import type { SOAPNotes, SOAPConfig } from '@/src/types/medical';
 ...
 interface SOAPNotesManagerProps { transcription?: string; initialNotes?: SOAPNotes; onNotesGenerated?: (notes: SOAPNotes) => void; onNotesUpdated?: (notes: SOAPNotes) => void; config?: SOAPConfig; className?: string; showActions?: boolean; editable?: boolean;
...
}, className = '', showActions = true, editable = false
}) => {
 ...
 const [downloadSuccess, setDownloadSuccess] = useState(false); // Generate SOAP notes from transcription const generateNotes = useCallback(async () => { if (!transcription) { setError('No transcription available to generate SOAP notes'); return; } setIsGenerating(true); setError(null); try {
...
${notes.plan}`; try {
  await navigator.clipboard.writeText(formattedNotes); setCopySuccess(true); setTimeout(() => setCopySuccess(false),
  2000);
} catch (err) { console.error('Failed to copy:', err); setError('Failed to copy to clipboard'); } }, [notes]); // Download notes as text file const downloadNotes = useCallback(() => { if (!notes) return;
 const formattedNotes = `SOAP Notes
${notes.timestamp ? `Generated: ${new Date(notes.timestamp).toLocaleString()}` : ''}
${notes.confidence ? `Confidence: ${(notes.confidence * 100).toFixed(0)}%` : ''} SUBJECTIVE:
${notes.subjective} OBJECTIVE:
