...
const extractChiefComplaint = (dialogue) => {
  const chief = dialogue.find(
    (sec) =>
      sec.section === 'Chief Complaint' || sec.section === 'Motivo de Consulta',
  );
  if (!chief) return '';
  const patientLine = chief.content.find(
    (line) => line.speaker === 'Paciente' || line.speaker === 'Patient',
  );
...
const getDialogueStructure = (t) => [
    ...
    content: [
      {
        ...
        text: 'Buenos días, María. ¿Cómo se siente hoy?',
      },
      {
        speaker: t('conversation.patient_speaker'),
        text: 'He tenido este dolor de cabeza persistente durante los últimos tres días.',
      },
    ],
    tags: [
  ...
  },
  {
    ...
    content: [
      {
        speaker: t('conversation.doctor_speaker'),
        text: '¿Puede describir el dolor? ¿Es pulsátil, punzante o sordo?',
      },
      {
        speaker: t('conversation.patient_speaker'),
        text: 'Es más bien un dolor sordo y constante, especialmente en el lado derecho de mi cabeza.',
      },
      {
        speaker: t('conversation.doctor_speaker'),
        text: 'En una escala del 1 al 10, ¿cómo calificaría el dolor?',
      },
      {
        speaker: t('conversation.patient_speaker'),
        text: 'Diría que es un 6 o 7. Definitivamente está afectando mis actividades diarias.',
      },
    ],
    tags: [
  ...
  },
  {
    ...
    content: [
      {
        speaker: t('conversation.doctor_speaker'),
        text: '¿Ha experimentado náuseas, vómitos o sensibilidad a la luz?',
      },
      {
        speaker: t('conversation.patient_speaker'),
        text: 'Sí, de hecho. Me siento un poco nauseosa, y las luces brillantes parecen empeorar el dolor.',
      },
    ],
    tags: [
  ...
  },
  {
    ...
    content: [
      {
        speaker: t('conversation.doctor_speaker'),
        text: '¿Alguna fiebre, rigidez de cuello o cambios en la visión?',
      },
      {
        speaker: t('conversation.patient_speaker'),
        text: 'No hay fiebre, pero mi cuello se siente un poco rígido. No tengo problemas de visión.',
      },
    ],
    tags: [
...
export function DialogueFlow({ onNext, onPrevious }) {
  ...
  const exchangeCount = dialogueStructure.reduce(
    (acc, sec) => acc + sec.content.length,
    0,
  );
  const tagCount = dialogueStructure.reduce(
    (acc, sec) => acc + sec.tags.length,
    0,
  );
      ...
      <div className="chief-complaint bg-blue-50 border-l-4 border-blue-500 p-4">
        <div className="flex justify-between items-center mb-2">
          <h3 className="font-semibold text-blue-900">
            {t('dialogue.sections.chief_complaint')}
          </h3>
      ...
      <Card className="bg-blue-50 border-blue-200">
        ...
        <CardContent>
          <div className="space-y-2 text-sm text-blue-800">
            <p>
              • Paciente presenta cefalea unilateral con fotofobia y náuseas
              asociadas
            </p>
            <p>
              • Duración de síntomas: 3 días, intensidad moderada a severa
              (6-7/10)
            </p>
            <p>
              • Se observa rigidez de cuello - considerar diferencial entre
