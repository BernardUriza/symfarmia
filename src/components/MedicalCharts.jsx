    ...
    <div className="h-64 flex items-end justify-center space-x-4 p-4">
      {data.data.map((item, index) => (
        <div key={index} className="flex flex-col items-center">
          ...
          ></div>
          <span className="text-xs text-gray-600 dark:text-gray-400 mt-2 text-center">{item.name}</span>
    ...
    <div className="h-64 flex items-end justify-center p-4">
      <div className="relative w-full h-full">
        <svg width="100%" height="100%" viewBox="0 0 400 200">
          ...
          {/* Data line */}
          <polyline
            points={data.data.map((d, i) => `${(i * 400) / (data.data.length - 1)},${200 - (d.patients / 200) * 180}`).join(' ')}
            fill="none"
            stroke="#3B82F6"
            strokeWidth="3"
          />
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-1">
        <div className="bg-white dark:bg-slate-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Chart Selection</h3>
      ...
      <div className="lg:col-span-2">
        <div className="bg-white dark:bg-slate-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{chartData.title}</h3>
