...
const TranscriptionSettings = ({ settings, onSettingChange }) => {
  ...
  const getConfidenceDescription = (threshold) => {
    ...
    if (threshold >= 70)
      return 'Acepta transcripciones buenas con pequeños errores';
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center space-x-2 mb-3">
          ...
          <h4 className="font-medium text-gray-900 ">Idioma y Región</h4>{' '}
        </div>{' '}
        <div className="space-y-3">
          ...
          {languages.map((language) => (
            <motion.button
              ...
              className={`w-full p-4 rounded-lg border-2 transition-all text-left ${settings.language === language.id ? 'border-blue-500 bg-blue-50 ' : 'border-gray-200 hover:border-blue-300'}`}
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <div
                  className={`w-4 h-4 rounded-full border-2 ${settings.language === language.id ? 'border-blue-500 bg-blue-500' : 'border-gray-300 '}`}
                >
                  {' '}
      ...
      <div className="space-y-4">
        ...
        <div className="p-4 bg-gray-50 rounded-lg">
          ...
          <div className="flex items-center justify-between mb-3">
            {' '}
            <span
              className={`text-sm font-medium ${getConfidenceColor(settings.confidenceThreshold)}`}
            >
              {' '}
          ...
          <input
            ...
            onChange={(e) =>
              onSettingChange('confidenceThreshold', parseInt(e.target.value))
            }
            className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer slider"
          />{' '}
          <div className="flex justify-between text-xs text-gray-500 mt-1">
            {' '}
            <span>50% (Permisivo)</span> <span>75% (Óptimo)</span>{' '}
        ...
        <div className="grid gap-4">
          ...
          {transcriptionServices.map((service) => (
            <motion.button
              key={service.id}
              onClick={() =>
                onSettingChange('transcriptionService', service.id)
              }
              className={`p-4 rounded-lg border-2 transition-all text-left ${settings.transcriptionService === service.id ? 'border-purple-500 bg-purple-50 ' : 'border-gray-200 hover:border-purple-300'}`}
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
              {' '}
              <div className="flex items-start justify-between">
                ...
                <div
                  className={`w-4 h-4 rounded-full border-2 flex-shrink-0 ${settings.transcriptionService === service.id ? 'border-purple-500 bg-purple-500' : 'border-gray-300 '}`}
                >
                  {' '}
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <motion.button
            ...
            className={`p-4 rounded-lg border-2 transition-all text-left ${settings.realtimeMode ? 'border-green-500 bg-green-50 ' : 'border-gray-200 hover:border-green-300'}`}
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
            {' '}
            <div className="text-center">
              ...
              <div className="font-medium text-gray-900 "> Tiempo Real </div>{' '}
              <div className="text-sm text-gray-600 mt-1">
                {' '}
          ...
          <motion.button
            ...
            className={`p-4 rounded-lg border-2 transition-all text-left ${!settings.realtimeMode ? 'border-blue-500 bg-blue-50 ' : 'border-gray-200 hover:border-blue-300'}`}
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
            {' '}
            <div className="text-center">
              {' '}
              <div className="text-2xl mb-2">📦</div>{' '}
              <div className="font-medium text-gray-900 "> Por Lotes </div>{' '}
              <div className="text-sm text-gray-600 mt-1">
                {' '}
        ...
        <div className="space-y-3">
          ...
          {[
            ...
            >
              ...
              <button
                onClick={() =>
                  onSettingChange(feature.key, !settings[feature.key])
                }
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${settings[feature.key] ? 'bg-indigo-600' : 'bg-gray-200 '}`}
              >
                {' '}
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${settings[feature.key] ? 'translate-x-6' : 'translate-x-1'}`}
                />{' '}
              </button>{' '}
            </div>
          ))}{' '}
