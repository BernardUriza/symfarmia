...
const AIAssistantPanel = () => {
  const { aiMessages, isAiThinking, finalTranscript, addAiMessage, logEvent } =
    useConsultation();
  ...
  const generateClinicalAlerts = useCallback((transcript) => {
    ...
    const redFlags = [
      {
        keyword: 'dolor torácico intenso',
        alert: 'Considerar síndrome coronario agudo',
        severity: 'high',
      },
      {
        keyword: 'dificultad respiratoria severa',
        alert: 'Evaluar distress respiratorio',
        severity: 'high',
      },
      {
        keyword: 'pérdida de conciencia',
        alert: 'Investigar causas neurológicas',
        severity: 'high',
      },
      {
        keyword: 'sangrado abundante',
        alert: 'Controlar signos vitales',
        severity: 'high',
      },
    ];
  ...
  const analyzeTranscriptForSuggestions = useCallback(
    async (transcript) => {
  ...
  const generateMedicalSuggestions = (query) => {
    ...
    if (
      queryLower.includes('tratamiento') ||
      queryLower.includes('medicación')
    ) {
      return [
            ...
            >
                ...
                {aiMessages.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-400 text-sm">
                      Te ayudo con análisis clínico, diagnósticos diferenciales
                  ...
                  aiMessages
                    ...
                    .map((message, index) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`flex items-start max-w-xs ${message.type === 'user' ? 'flex-row-reverse' : ''}`}
                          >
                            <div
              ...
              <div className="border-t border-gray-100 p-4 sticky bottom-0 bg-white shadow-md">
                ...
                <div className="mt-3 flex flex-row flex-wrap gap-2 action-buttons">
                  <button
                    onClick={() =>
                      handleSuggestionClick('Analizar síntomas principales')
                    }
                  ...
                  <button
                    onClick={() =>
                      handleSuggestionClick('Revisar tratamientos recomendados')
                    }
                    className="flex items-center px-3 py-1 text-xs bg-gray-100 text-gray-700 rounded-full hover:bg-gray-200 transition-colors"
                  >
              ...
              {suggestions.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-400 text-sm mt-2">
                    Las sugerencias aparecerán conforme se analice la
                    transcripción
                  </p>
                </div>
              )}
              ...
              {finalTranscript && (
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    <div>
                      Palabras totales: {finalTranscript.split(' ').length}
                    </div>
                    <div>
                      Duración estimada:{' '}
                      {Math.ceil(finalTranscript.split(' ').length / 150)} min
                    </div>
