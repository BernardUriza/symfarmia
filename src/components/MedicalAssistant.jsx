...
const MedicalAssistant = () => {
  ...
  const handleSendMessage = async () => {
    ...
    setMessage('');
    // Add user message addChatMessage({ type: 'user', content: userMessage, sender: 'user' });
    setIsTyping(true);
    try {
      // Get AI response
      ...
      // Add AI response addChatMessage({ type: 'assistant', content: aiResponse.response, sender: 'ai', mode: assistantMode });
    } catch (error) {
  ...
  const quickCommands = [
    {
      id: 'summarize',
      text: '/summarize visit',
      icon: DocumentTextIcon,
      desc: 'Summarize current patient visit',
    },
    {
      id: 'interactions',
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {modeButtons.map((mode) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors flex items-center space-x-1 ${assistantMode === mode.id ? `bg-${mode.color}-100 text-${mode.color}-800 border border-${mode.color}-200` : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                >
                  {' '}
            ...
            <div className="p-4 border-b border-gray-200 ">
              ...
              <div className="space-y-2">
                ...
                {clinicalAlerts.slice(0, 3).map((alert) => (
                  <div
                    key={alert.id}
                    className={`p-2 rounded-lg text-xs ${alert.priority === 'high' ? 'bg-red-50 text-red-800' : 'bg-yellow-50 text-yellow-800'}`}
                  >
                    {' '}
              ...
              chatHistory.map((msg) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`max-w-xs px-4 py-2 rounded-lg border ${getMessageColor(msg.type)} ${msg.sender === 'user' ? 'bg-blue-600 text-white border-blue-600' : ''}`}
                    >
                      {' '}
          ...
          <AnimatePresence>
            ...
            {showCommands && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 20 }}
                className="absolute bottom-20 left-4 right-4 bg-white rounded-lg shadow-xl border border-gray-200 max-h-64 overflow-y-auto"
              >
                {' '}
                <div className="p-2">
                  {' '}
                  ...
                  {quickCommands.map((cmd) => (
                    <button
                      ...
                      className="w-full text-left p-2 rounded-lg hover:bg-gray-100 transition-colors flex items-center space-x-3"
                    >
                      {' '}
            ...
            <div className="flex space-x-2">
              ...
              <div className="flex-1 relative">
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white text-gray-900 placeholder-gray-600 "
                  rows={2}
                />{' '}
                {message.startsWith('/') && (
                  <button
