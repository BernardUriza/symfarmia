...
const SmokeTest = () => {
    ...
    try {
      ...
      try {
        ...
        if (healthResponse.success && healthResponse.status === 'healthy') {
          addTest('Health Check', 'passed', `API healthy with ${healthResponse.counts.patients} patients`)
        } else {
          addTest('Health Check', 'failed', null, 'API not responding correctly')
        }
      } catch (error) {
      ...
      try {
        ...
        if (patientsResponse.success && Array.isArray(patientsResponse.data)) {
          addTest('Get Patients', 'passed', `Retrieved ${patientsResponse.data.length} patients`)
        } else {
          addTest('Get Patients', 'failed', null, 'Invalid patients data structure')
        }
      } catch (error) {
      ...
      try {
        ...
        if (reportsResponse.success && Array.isArray(reportsResponse.data)) {
          addTest('Get Medical Reports', 'passed', `Retrieved ${reportsResponse.data.length} reports`)
        } else {
          addTest('Get Medical Reports', 'failed', null, 'Invalid reports data structure')
        }
      } catch (error) {
      ...
      try {
        ...
        if (studyTypesResponse.success && Array.isArray(studyTypesResponse.data)) {
          addTest('Get Study Types', 'passed', `Retrieved ${studyTypesResponse.data.length} study types`)
        } else {
          addTest('Get Study Types', 'failed', null, 'Invalid study types data structure')
        }
      } catch (error) {
      ...
      try {
        ...
        if (createResponse.success && createResponse.data.id) {
          addTest('Create Patient', 'passed', `Created patient with ID ${createResponse.data.id}`)
        } else {
      ...
      try {
        ...
        if (testElement) {
          addTest('Component Rendering', 'passed', 'SmokeTest component rendered successfully')
        } else {
          addTest('Component Rendering', 'failed', null, 'Component not found in DOM')
        }
      } catch (error) {
      ...
      try {
        ...
        if (unsupported.length === 0) {
          ...
          addTest('Browser Features', 'failed', null, `Missing features: ${unsupported.map(([name]) => name).join(', ')}`)
        }
      } catch (error) {
  ...
  return (
    <div id="smoke-test-container" className="fixed bottom-4 right-4 bg-white rounded-lg shadow-xl border border-gray-200 p-6 max-w-md w-96 z-50">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold text-gray-900">Frontend Smoke Test</h3>
        <button
          ...
          <div className="space-y-3">
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              <div className="flex space-x-4 text-sm">
                <span className="text-green-600">✓ {results.passed} passed</span>
            ...
            <div className="max-h-48 overflow-y-auto space-y-2">
              {results.tests.map((test, index) => (
                <div key={index} className="flex items-start space-x-2 text-sm">
                  <span className={`flex-shrink-0 ${test.status === 'passed' ? 'text-green-600' : 'text-red-600'}`}>
                    {test.status === 'passed' ? '✓' : '✗'}
                  </span>
                  <div>
                    <div className="font-medium">{test.name}</div>
                    {test.details && <div className="text-gray-600">{test.details}</div>}
                    {test.error && <div className="text-red-600 text-xs">{test.error}</div>}
                  </div>
                </div>
              ))}
            </div>
