  ...
  const getTargetStatus = () => {
    ...
    if (percentage >= 100)
      return {
        ...
        text: t('productivity.target_achieved'),
      };
    if (percentage >= 80)
      return {
        icon: Target,
        color: 'text-orange-600',
        text: t('productivity.near_target'),
      };
    return {
      icon: TrendingUp,
      color: 'text-blue-600',
      text: t('productivity.improving'),
    };
  };
  const targetStatus = getTargetStatus();
  return (
    <div
      className={`p-4 rounded-xl border-2 transition-all duration-200 hover:shadow-lg ${colorClasses[color]}`}
    >
      {' '}
      <div className="flex items-center justify-between mb-3">
        ...
        {targetStatus && (
          <div className="flex items-center space-x-1">
            {' '}
            <targetStatus.icon
              className={`h-4 w-4 ${targetStatus.color}`}
            />{' '}
          </div>
        )}{' '}
      </div>{' '}
        ...
        <div>
          ...
          {trend && (
            ...
            >
              ...
              <span>
                {Math.abs(trend)}
                {unit && !isEconomic ? unit : ''}
              </span>{' '}
              <span className="opacity-75">
                vs {t('productivity.previous_period')}
              </span>{' '}
            </div>
          )}{' '}
        </div>{' '}
          ...
          <div className="flex items-center justify-between">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 ">
                {t('productivity.daily_additional_revenue')}
              </p>{' '}
              <p className="text-xl font-bold text-emerald-700 ">
                {' '}
                {formatCurrency(
                  metrics.economicImpact.dailyAdditionalRevenue,
                )}{' '}
              </p>{' '}
            </div>{' '}
        ...
        <div className="bg-white rounded-lg p-4 border border-emerald-200 ">
          ...
          <div className="flex items-center justify-between">
            ...
            <div>
              {' '}
              <p className="text-sm text-gray-600 ">
                {t('productivity.monthly_additional_revenue')}
              </p>{' '}
              <p className="text-xl font-bold text-emerald-700 ">
                {' '}
                {formatCurrency(
                  metrics.economicImpact.monthlyAdditionalRevenue,
                )}{' '}
              </p>{' '}
            </div>{' '}
            <div className="text-2xl">📈</div>{' '}
        ...
        <div className="bg-white rounded-lg p-4 border border-emerald-200 ">
          ...
          <div className="flex items-center justify-between">
            ...
            <div>
              {' '}
              <p className="text-sm text-gray-600 ">
                {t('productivity.additional_consultations')}
              </p>{' '}
              <p className="text-xl font-bold text-emerald-700 ">
                {' '}
      ...
      <div className="mt-4 p-4 bg-emerald-100 rounded-lg">
        ...
        <div className="flex items-center space-x-2">
          ...
          <p className="text-sm text-emerald-700 ">
            {' '}
            <strong>{t('productivity.key_insight')}</strong>{' '}
            {t('productivity.time_saved_message', {
              hours: metrics.coreMetrics.timeSaved.hours,
            })}{' '}
          </p>{' '}
        </div>{' '}
      </div>{' '}
...
function MedicalProductivityMetrics({
  timeRange = 'today',
  showEconomicMetrics = true,
}) {
  const { t } = useI18n();
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Economic Impact Section */}{' '}
      {showEconomicMetrics && <EconomicImpact metrics={metrics} />}{' '}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200 ">
            ...
            <p className="text-sm text-blue-700 ">
              {' '}
              {t('productivity.productivity_gain_message', {
                percentage: metrics.insights.productivityGain,
              })}{' '}
            </p>{' '}
          </div>{' '}
          <div className="bg-green-50 rounded-lg p-4 border border-green-200 ">
            ...
            <p className="text-sm text-green-700 ">
              {' '}
              {t('productivity.capacity_increase_message', {
                percentage: metrics.insights.capacityIncrease,
              })}{' '}
            </p>{' '}
          </div>{' '}
        </div>{' '}
