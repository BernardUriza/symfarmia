...
import { useDemoPatients } from "../../src/domains/demo/hooks/useDemoPatients"; // EXACT Figma workflow steps as specified
const getMedicalWorkflowSteps = (t) => [
  {
    id: "escuchar",
    label: t("workflow.steps.listen"),
    icon: Mic,
    component: ConversationCaptureWrapper,
  },
  {
    id: "revisar",
    label: t("workflow.steps.review"),
    icon: MessageSquare,
    component: DialogueFlow,
  },
  {
    id: "notas",
    label: t("workflow.steps.notes"),
    icon: FileText,
    component: ClinicalNotes,
  },
  {
    id: "ordenes",
    label: t("workflow.steps.orders"),
    icon: ClipboardList,
    component: OrderEntry,
  },
  {
    id: "resumen",
    label: t("workflow.steps.summary"),
    icon: Download,
...
function MedicalAIDemo() {
  ...
  // LocalStorage: cargar paciente después de hidratar
  useEffect(() => {
    const storedPatient = localStorage.getItem("medicalAIDemoPatient");
    if (storedPatient) {
      try {
        setExternalPatient(JSON.parse(storedPatient));
  ...
  // LocalStorage: limpiar paciente después de usarlo
  useEffect(() => {
      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="p-6">
          <div className="max-w-2xl mx-auto">
            <div className="card-gradient glass rounded-xl shadow-lg border border-border p-6 animate-slide-up">
              ...
              <div className="space-y-3">
                {patients.map((patient) => (
                  <button
                    ...
                    className="w-full text-left p-4 border border-border rounded-lg hover:border-primary hover:bg-primary/5 transition-smooth hover-scale"
                  >
                    <div className="flex items-center justify-between">
                      <div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            >
              ...
              {isRecording
                ? t("workflow.actions.stop_recording")
                : t("workflow.actions.start_recording")}
            </Button>
          </div>
        </div>
        <div className="mt-4">
              ...
              return (
                <div key={step.id} className="flex items-center gap-2">
                  ...
                  {index < steps.length - 1 && (
                    <div
                      className={`w-8 h-0.5 ${index < currentStepIndex ? "bg-green-500" : "bg-slate-300"}`}
                    />
                  )}
                </div>
              );
        ...
        <div className="w-80 bg-background border-r border-border min-h-screen">
          <div className="p-6">
            <div className="mb-8">
              ...
              <div className="card-float glass p-6 space-y-4">
            ...
            <div className="mb-8">
              <h3 className="text-sm font-semibold text-foreground mb-4">
                {t("workflow.medical_navigation")}
              </h3>
              <nav className="space-y-2">
                <button className="w-full flex items-center gap-3 px-4 py-3 glass text-secondary rounded-lg border border-secondary/20 hover:border-secondary/40 transition-smooth hover-scale">
                  <Users className="h-4 w-4" />
                  <span className="text-sm font-medium">
                    {t("workflow.navigation.view_all_patients")}
                  </span>
                </button>
                <button className="w-full flex items-center gap-3 px-4 py-3 text-foreground/70 rounded-lg hover:bg-muted/50 transition-smooth hover-scale">
                  ...
                  <span className="text-sm font-medium">
                    {t("workflow.navigation.medical_reports")}
                  </span>
                </button>
                <button className="w-full flex items-center gap-3 px-4 py-3 text-foreground/70 rounded-lg hover:bg-muted/50 transition-smooth hover-scale">
                  <FileText className="h-4 w-4" />
                  <span className="text-sm font-medium">
                    {t("workflow.navigation.clinical_history")}
                  </span>
                </button>
                <button className="w-full flex items-center gap-3 px-4 py-3 text-foreground/70 rounded-lg hover:bg-muted/50 transition-smooth hover-scale">
                  <Activity className="h-4 w-4" />
                  <span className="text-sm font-medium">
                    {t("workflow.navigation.new_consultation")}
                  </span>
                </button>
              </nav>
            </div>
            <div className="stat-card glass">
              ...
              <div className="text-sm text-foreground/60">
                {currentStepIndex === 0 &&
                  t("workflow.step_descriptions.listen")}
                {currentStepIndex === 1 &&
                  t("workflow.step_descriptions.review")}
                {currentStepIndex === 2 &&
                  t("workflow.step_descriptions.notes")}
                {currentStepIndex === 3 &&
                  t("workflow.step_descriptions.orders")}
                {currentStepIndex === 4 &&
                  t("workflow.step_descriptions.summary")}
              </div>
            </div>
          </div>
        </div>
