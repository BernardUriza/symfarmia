...
const TimeTracker = () => {
  ...
  const fetchDoctorStats = async () => {
    ...
    const mockDoctorStats = [
      ...
      { name: 'Dr. Martínez', avgTime: 11.8, consultations: 20, efficiency: 95 },
      { name: 'Dra. Rodríguez', avgTime: 14.2, consultations: 12, efficiency: 88 }
    ];
  ...
  const stopSession = () => {
    if (currentSession.startTime) {
      ...
      // Here you would save the session data to your backend
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Sesión Actual</h3>
          <div className={`flex items-center space-x-2 ${currentSession.isActive ? 'text-green-600 dark:text-green-400' : 'text-gray-400'}`}>
            <div className={`w-2 h-2 rounded-full ${currentSession.isActive ? 'bg-green-500 animate-pulse' : 'bg-gray-400'}`}></div>
            <span className="text-sm">{currentSession.isActive ? 'Activa' : 'Inactiva'}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-600 dark:text-gray-400">Tipo de Consulta</span>
        ...
        <div className="flex space-x-3">
          ...
          {!currentSession.isActive && currentSession.startTime && (
            <button
              onClick={() => setCurrentSession(prev => ({ ...prev, isActive: true }))}
              className="flex items-center space-x-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"
            >
              <Play className="h-4 w-4" />
              <span>Reanudar</span>
      ...
      {/* Today's Metrics */}
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Métricas del Día</h3>
        
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{todayMetrics.totalConsultations}</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">Consultas</div>
          </div>
          
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600 dark:text-green-400">{todayMetrics.averageTime} min</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">Promedio</div>
          </div>
          
          <div className="text-center">
            <div className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{todayMetrics.totalTime} min</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">Tiempo Total</div>
          </div>
          
          <div className="text-center">
            <div className={`text-2xl font-bold ${getEfficiencyColor(todayMetrics.efficiency)}`}>
      ...
      {/* Weekly Trend */}
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Tendencia Semanal</h3>
            ...
            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400">{day.consultations} consultas</div>
                </div>
              </div>
              <div className="text-right">
                <div className="text-sm font-medium text-gray-900 dark:text-white">{day.avgTime} min</div>
      ...
      {/* Doctor Performance */}
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Rendimiento por Médico</h3>
            ...
            <div key={index} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400">{doctor.consultations} consultas</div>
                </div>
              </div>
              
              <div className="flex items-center space-x-4">
                <div className="text-right">
                  <div className="text-sm font-medium text-gray-900 dark:text-white">{doctor.avgTime} min</div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">promedio</div>
                </div>
                
                <div className={`flex items-center space-x-1 ${getEfficiencyColor(doctor.efficiency)}`}>
                  {getEfficiencyIcon(doctor.efficiency)}
                  <span className="text-sm font-medium">{doctor.efficiency}%</span>
                </div>
              </div>
            </div>
          ))}
