        ...
        <CardContent>
          <div className="space-y-4 max-h-96 overflow-y-auto" role="log" aria-live="polite" aria-label="Medical conversation transcript">
            {transcription?.segments.map((segment, index) => (
              <div key={index} className="flex gap-4 p-3 rounded-lg bg-slate-50 dark:bg-gray-700 shadow-sm border-2 border-gray-200 dark:border-gray-600 leading-relaxed" role="article" aria-labelledby={`speaker-${index}`}>
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-xs text-slate-500 dark:text-gray-400" aria-label={`Time: ${new Date(segment.startTime).toLocaleTimeString('es-ES')}`}>{new Date(segment.startTime).toLocaleTimeString('es-ES')}</span>
                </div>
                <p className="flex-1 text-slate-700 dark:text-gray-300 font-medium" aria-label={`${segment.speaker} says: ${segment.text}`}>{segment.text}</p>
              </div>
            ))}
            {status === TranscriptionStatus.PROCESSING && (
              <div className="flex gap-4 p-3 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700">
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-xs text-slate-500 dark:text-gray-400">{t('conversation.processing.processing_status')}</span>
                </div>
